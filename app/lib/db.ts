...
export async function initDatabase() {
  ...
  // 2. Seed / sync Default Organizations using emails from .env.local
  const alphaEmail = process.env.ALPHA_EMAIL || "alpha@example.com";
  const betaEmail = process.env.BETA_EMAIL || "beta@example.com";
  const alphaOrgName = process.env.ALPHA_ORG_NAME || "Organization Alpha";
  const betaOrgName = process.env.BETA_ORG_NAME || "Organization Beta";

  // Upsert so org rows are created on first run AND updated if emails change in .env.local
  await query(`
      INSERT INTO organizations (id, name, email) VALUES
      ('org-a', $1, $2),
      ('org-b', $3, $4)
      ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, email = EXCLUDED.email
    `, [alphaOrgName, alphaEmail, betaOrgName, betaEmail]);
  console.log(`[DB Seed] Orgs synced — Alpha: ${alphaEmail}, Beta: ${betaEmail}`);

  // 3. Seed 500 rows for Organization Alpha ('org-a') if it's empty
  const dataCheck = await query("SELECT COUNT(*) FROM organization_data WHERE org_id = 'org-a'");
  const currentCount = parseInt(dataCheck.rows[0].count, 10);

  if (currentCount === 0) {
    console.log("[DB Seed] Database empty. Bulk-seeding 500 enterprise rows for Organization Alpha...");
    ...
    // Insert optimal slices to avoid query parameter limit limit in Postgres (max 65535, we have 3500)
    // Slices of 100 rows per transaction is extremely optimized
    const chunkSize = 100;
    for (let chunkIdx = 0; chunkIdx < insertValues.length; chunkIdx += chunkSize) {
      ...
      // Adjust parameter indexes for the query slice
    ...
    console.log("[DB Seed] Bulk-seed successfully finished!");
  } else {
    console.log(`[DB Seed] Data tables already populated. (Org-A current rows = ${currentCount})`);
  }

  console.log("[DB Init] Schema auto-migration complete!");
}
